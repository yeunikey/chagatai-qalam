...
const switchInput =
  "relative h-[26px] w-[52px] flex-none appearance-none rounded-full bg-[#e7e8ea] shadow-[inset_0_0_0_2px_#f2f3f4] transition-colors after:absolute after:left-[3px] after:top-[3px] after:h-5 after:w-5 after:rounded-full after:bg-[#b9bdc2] after:shadow-[0_1px_4px_rgba(20,30,40,0.18)] after:transition-transform checked:bg-[#d8f1e6] checked:after:translate-x-[26px] checked:after:bg-[#0b8a61]";
const optionTool =
  "group relative border border-[#dfe3e7] flex min-h-[40px] items-center justify-between gap-3 rounded-xl px-3 text-[#25282c] transition-colors";
const toolLabel =
  "inline-flex min-w-0 flex-1 items-center gap-2.5 text-sm font-medium";
const tooltip =
  "pointer-events-none absolute inset-x-0 top-[calc(100%+6px)] z-30 rounded-md border border-[#dfe3e7] bg-[#20242a] px-3 py-2 text-xs font-semibold leading-snug text-white opacity-0 shadow-[0_10px_24px_rgba(20,30,40,0.18)] transition-opacity group-hover:opacity-100 group-focus-within:opacity-100";
  ...
  return (
    <aside
      className={`relative flex-none overflow-visible transition-[width] duration-300 ease-out ${
        isOpen ? "w-[360px] max-[1280px]:w-full" : "w-0 max-[1280px]:h-0"
      }`}
      aria-hidden={!isOpen}
    >
      <div
        className={`h-full w-[360px] overflow-auto rounded-2xl border border-[#edf0f2] bg-white p-5 transition-[opacity,transform] duration-300 ease-out max-[1280px]:w-full max-[860px]:px-[18px] ${
          isOpen
      ...
      >
        <div className="mb-3 flex items-center justify-between gap-3 pr-10">
          <h2 className="text-sm font-semibold text-[#6f747a]">Настройки</h2>
        </div>
        ...
        <div className="grid gap-2">
          <label className={optionTool}>
            <span className={toolLabel}>
              <Highlighter
                className="flex-none text-[#6f747a]"
                size={16}
                strokeWidth={2.2}
              />
        ...
        <h2 className="mb-3 mt-7 text-sm font-semibold text-[#6f747a]">
