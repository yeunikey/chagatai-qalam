...
export default function App() {
  ...
  return (
    <div className="min-h-[calc(100vh-72px)] bg-background px-4 py-6 pb-24 font-sans text-slate-900 md:px-8">
      <div
        className={`mx-auto grid max-w-7xl grid-cols-1 ${showBookmarks ? "lg:grid-cols-4" : "lg:grid-cols-1"} items-start gap-8`}
      >
        {showBookmarks && (
          <BookmarksWidget
            bookmarks={bookmarks}
        ...
        <main
          className={`order-1 flex flex-col gap-8 lg:order-2 ${showBookmarks ? "lg:col-span-3" : "lg:col-span-1"}`}
        >
          <EditorWidget
            inputText={inputText}
            fontSize={fontSize}
